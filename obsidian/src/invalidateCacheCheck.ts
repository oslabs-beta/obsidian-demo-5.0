...
export function isMutation(gqlQuery: { query: any }): boolean {
  ...
  checkMutationVisitor.OperationDefinition(ast);
  console.log(
    `isMutation function in Invalidate cache. isMutation= `,
    isMutation
  );
...
export async function invalidateCache(
  ...
  console.log('inside invalidateCache function');
  for (const redisKey in normalizedMutation) {
    ...
    if (
      ...
      console.log('run cacheDelete');
      await cache.cacheDelete(redisKey);
    } else {
      console.log('else, should be running cache.cacheWriteObject');
